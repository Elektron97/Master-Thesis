...
%         options = optimoptions('fsolve','Algorithm',' trust-region-dogleg','MaxFunctionEvaluations',2e10);%,'Jacobian','on'); 'trust-region-dogleg' (default), 'trust-region', and 'levenberg-marquardt'.
        options = optimoptions('fsolve','Algorithm','levenberg-marquardt','MaxFunctionEvaluations',2e10);%,'Jacobian','on'); 'trust-region-dogleg' (default), 'trust-region', and 'levenberg-marquardt'.
        if ~eq_feedback
            for i = 1:length(tau)
    ...
    %         save("equilibria_tau.mat", "equilibria");
           ...
           for i = 1:length(theta_rd)
                equilibria_handle = @(q) Equilibrium(T1, [theta_rd(i); q], 0, 1);
    
                for j = 1:n_try
                    equilibria{i}(:, j) = fsolve(equilibria_handle, randn(2, 1), options);
                end
                disp("End of " + num2str(theta_rd(i)) + " [rad] Equilibria");
            end
            save("equilibria_FL.mat", "equilibria");
        end
    else
        load("equilibria_tau.mat");
%           load("equilibria_FL.mat");
        %% Filter Equilibria
...
%         for i = 1:length(theta_rd)
%         
%             plot(theta_rd(i), filtered_equilibria{i}(1, :), 'x', 'Color', [0 0.4470 0.7410])
%         end
%         hold off
%         grid on
%         xlabel("\tau")
...
%         for i = 1:length(theta_rd)
%         
%             plot(theta_rd(i), filtered_equilibria{i}(2, :), 'x', 'Color', [0.8500 0.3250 0.0980])
%         end
%         hold off
%         grid on
%         xlabel("\tau")
